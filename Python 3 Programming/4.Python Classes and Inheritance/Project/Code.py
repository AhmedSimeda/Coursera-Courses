...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        ...
        return r > self.difficulty
    
    def getPossibleLetters(self, guessed):
        LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
        #removing guessed letters
        for g in guessed:
            LETTERS = LETTERS.replace(g,'')
        #if there is no enough money to guess a vowel
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if decision:
            PossibleLetters = sorted(list(PossibleLetters), reverse=True, key=lambda x:self.SORTED_FREQUENCIES.index(x))
